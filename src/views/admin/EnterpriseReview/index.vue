<template>
  <div class="enterprise-review">
    ...
    <!-- 详情对话框 -->
    <el-dialog v-model="detailVisible" title="企业详情" width="60%">
      <el-descriptions :column="2" border>
        <el-descriptions-item label="企业ID">{{ currentEnterprise.enterpriseId }}</el-descriptions-item>
        ...
        <el-descriptions-item label="申请时间" :span="2">{{ currentEnterprise.createdAt }}</el-descriptions-item>
      </el-descriptions>
    </el-dialog>

    <!-- 拒绝对话框 -->
    <el-dialog v-model="rejectVisible" title="拒绝企业认证" width="500px">
      <el-form>
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getAdminPendingEnterprisesAPI({
...
const handleApprove = async (row) => {
  await ElMessageBox.confirm(`确认审核通过企业「${row.enterpriseName}」？`, '审核确认', {
    confirmButtonText: '确认通过',
    cancelButtonText: '取消',
    type: 'success'
  })
  try {
    await approveEnterpriseAPI(row.enterpriseId)
